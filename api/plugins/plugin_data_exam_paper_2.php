...
function plugin_data_exam_paper_2_edit_default($id)  {
    ...
    foreach($题库抽取 AS $题目类型=>$对应题目) {
        ...
        foreach($对应题目 AS $单个题目)     {
            ...
            $edit_default[$题目类型][] = ['name' => "题目_".$单个题目['id'], 'show'=>true, 'type'=>'radiogroup', 'options'=>$题目选项, 'label' => $序号."、".$单个题目['题干'], 'value' => "A", 'placeholder' => "", 'helptext' => "", 'rules' => ['required' => true, 'disabled' => false, 'xs'=>12, 'sm'=>12]];
    ...
    $RS['edit_default']['titletext']  = __("开始您的练习");
    $RS['edit_default']['titlememo']  = __("不限制时间,每次随机出题");

...
<h1 align="center" style="margin: 30px 0 30px; font-weight: bold;">智慧学工管理系统 v0.1</h1>
<h4 align="center">基于React和PHP开发的快速开发框架</h4>

## 平台简介

智慧学工管理系统是单点科技开发的,针对高校,职校的一款学生工作管理系统,基于GPL和商业许可双重授权发布。
...
### 德育量化(学生积分)
1.  指标设置：一级指标，二级指标，积分项目，调用函数（支持把其它模块的数据同步到某一个积分项目），参数设置，代录设置（支持设置某个学生针对整个班级学生录入某个积分项目的数据，用于减少班主任的工作量）。
2.  指标查询：一级指标，二级指标，积分项目，调用函数，代录查询。
3.  个人积分：班主任管理，系部审核，学工审核，学校审核，明细管理，明细查询，二级管理（可以设置某个用户做为某些积分项目的管理员，只能录入被授权的积分项目）。
...
后端安装：
```
后端代码在api目录下面，需要PHP7和MYSQL5.6以上支持。服务器脚本随后放出。
```
